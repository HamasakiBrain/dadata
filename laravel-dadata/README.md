...
Запустить:
```bash
composer require "HamasakiBrain/laravel-dadata"
```
---
__Для Laravel < 5.5:__
Зарегистрировать service-provider в config/app.php:
```php
  HamasakiBrain\Dadata\DadataServiceProvider::class,
```
...
Опубликовать конфиг: 
```bash
php artisan vendor:publish --provider="HamasakiBrain\Dadata\DadataServiceProvider"
```
...
```php
use HamasakiBrain\Dadata\Facades\DadataSuggest;
```
...
Добавить в клас фасад:
```php
use HamasakiBrain\Dadata\Facades\DadataClean;
```
Использовать методы: 
```php
